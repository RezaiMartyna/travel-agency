...
describe('Component OrderOption', () => {
  it('should render without crashing', () => {
    ...
    const component = shallow(<OrderOption type='exemplefortype' name='exempleforname' />);
...
for (let type in optionTypes) {
  describe(`Component OrderOption with type=${type}`, () => {
    ...
    switch (type) {
      ...
      case 'icons': {
        /* tests for div with icons */
        it('contains div with icons', () => {
          const mainDiv = renderedSubcomponent.find('.component');
          expect(mainDiv.length).toBe(1);
          const emptyOption = mainDiv
            .find('.icon')
            .find('Icon[name="times-circle"]').length;
          expect(emptyOption).toBe(1);
      ...
      case 'number': {
        it('contains div with input', () => {
          ...
          expect(mainDiv.length).toBe(1);
          const option = mainDiv.find('.inputSmall');
      ...
      */
      case 'text': {
        it('contains div with input', () => {
          const mainDiv = renderedSubcomponent.find('.component');
          expect(mainDiv.length).toBe(1);
          const option = mainDiv.find('.input');
          expect(option.length).toBe(1);
        });
      ...
      */
      case 'date': {
        it('contains div with DatePicker', () => {
          const mainDiv = renderedSubcomponent.find('.component');
          expect(mainDiv.length).toBe(1);
          const option = mainDiv.find(DatePicker);
          expect(option.length).toBe(1);
        });
        it('should run setOrderOption function on change', () => {
